...
/// </summary>
public class PuzzleGameNetworkRoomPlayer : NetworkRoomPlayer
{
    ...
    public override void OnStartServer() 
    {
        ...
        //PuzzleGameNetworkRoomManager.singleton.OnAddPlayer += RpcUpdatePlayerList;
    }
    ...
    {
        UpdatePlayerList(PuzzleGameNetworkRoomManager.singleton.roomSlots);
    }
    ...
    {
        if (!isServer) return;
        PuzzleGameNetworkRoomManager.singleton.ServerChangeScene(PuzzleGameNetworkRoomManager.singleton.GameplayScene);
    }
    ...
    public void SetStartInteractible(bool isInteractible)
    {
        if (isServer)
        {
            startButton.interactable = isInteractible;
        }
    }
    ...
    public void UpdatePlayerList(HashSet<NetworkRoomPlayer> players)
    {
        ...
        List<PuzzleGameNetworkRoomPlayer> roomPlayers = playerList.GetComponentsInChildren<PlayerReadyState>().Select(readyState => readyState.RoomPlayer).ToList();
        ...
        foreach(NetworkRoomPlayer player in players.Where(roomPlayer => !roomPlayers.Contains(roomPlayer)))
        {
            ...
            obj.GetComponent<PlayerReadyState>().SetRoomPlayer((PuzzleGameNetworkRoomPlayer)player);
        }
